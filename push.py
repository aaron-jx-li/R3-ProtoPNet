...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    # Initialize prototype receptive field boxes and bounding boxes
    ...
    if bounding_box_tracker is not None:
        bounding_box_tracker = proto_bound_boxes
    end = time.time()
    log('\tpush time: \t{0}'.format(end -  start))
    return bounding_box_tracker
...
def update_prototypes_on_batch(search_batch_input,
    ...
    # inputs to the last layer; distances to prototypes
    protoL_input_ = np.copy(protoL_input_torch.detach().cpu().numpy())
    proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy())

    del protoL_input_torch, proto_dist_torch

    # dict format is {class_number: list of image indices}
    if class_specific:
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            # update the global min distances and feature map patches (on conv outputs)
