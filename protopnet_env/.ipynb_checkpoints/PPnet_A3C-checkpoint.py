...
class PPnet_A3C(PPO):
    ...
    #Most of work is done here: ask the env to send a obs_batch, 
    #and then exploit the policy network (PPnet) to get an action (heatmaps of the batch), 
    #computing gradients along the way,
    #and finally update the policy network with PPO
    
    ## Modify PPO specific gradients and losses
    def training_step(self):
        obs_batch = self.env.get_obs()
        action = self.policy.compute_action(obs_batch)
        reward = self.env.reward_fn(action)
        
        self.get_policy().update(reward)
...
# Either inherit from Algorithm or A3C?
class PPnet_A3C(Algorithm):
    @classmethod
    @override(Algorithm)
...
# This A3C config class can be largely kept
